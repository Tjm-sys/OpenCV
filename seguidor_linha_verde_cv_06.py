...
while True:
    ...
    erro_pos_suave = 0 #variaveis para o calculo do controle
    erro_ang_suave = 0
    ...
    if contours_black:
        ...
        if cv2.contourArea(c) > 500:
            ...
            derivativo = erro_pos - erro_pos_ant
            erro_pos_suave = (1 - alpha) * erro_pos_suave + alpha * erro_pos #'filtro' deixa o movimento mais continuo
            erro_ang_suave = (1 - alpha) * erro_ang_suave + alpha * angle 

            #K = constante que define o quanto que reajimos ao erro
            ...
            Kd = 0.1 #AJUSTAR: -se demorar para reajir Kd = Derivativo         

            #calculo da correção com PD e 'filtro'
            correcao = (
                Kp_pos * erro_pos_suave +
                Kp_ang * erro_ang_suave +
                Kd * derivativo
            )
            ...
            vel_base = 120 #AJUSTAR velocidade
            motor_esq_mov = vel_base - correcao
