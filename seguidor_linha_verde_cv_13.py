...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640) #regula a resolução (qualidade da imagem)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480) #código mais rápido assim
...
correcao_ant = 0 #variavel para guardar a última correção aplicada, caso a linha não seja detectada
...
while True:
    ...
    if contours_black:
        ...
        if cv2.contourArea(c) > 500:
            ...
            correcao_ant = correcao #salva a correção anterior
            ...
            #CRUZAMENTO: lógica para detectar
            (w, h) = rect[1]
            ...
            cv2.putText(frame, f"Correção: {correcao:.2f}", (20,180),cv2.FONT_HERSHEY_COMPLEX, 0.7, (0,0,0), 1)
            ...
            correcao = correcao_ant #se a linha não for detectada, mantém a última correção aplicada
            cv2.putText(frame, "Linha: Não detectada", (20,30),cv2.FONT_HERSHEY_COMPLEX, 0.7, (0,0,0), 1)
    ...
    if contours_green:
        for c in contours_green:
            ...
            if area > 500: #AJUSTAR a área/tamanho min do verde
                ...
                anglev = rectv[2] #ângulo do verde
        ...
        if len(centros_verdes) == 1:
            ...
            if cx_unico > cl: #testa em qual lado o verde está em relação à linha
                if cruzamento == True: #lógica para anular verdes caso haja interseção
        ...
        if len(centros_verdes) == 2:
            if cruzamento == True: #lógica para anular dois verdes caso haja interseção
                dir = "Dois anulado"
            elif cruzamento == False: 
                dir = "Dois verdes"
