...
while True:
    ...
    if contours_black:
        ...
        if cv2.contourArea(c) > 500:
            ...
            correcao = Kp_pos * erro_pos_suave + Kp_ang * erro_ang_suave #***calcula a correção separando os ganhos
            correcao = max(min(correcao, 100), -100) #***limita a correção

            vel_base = 120 #AJUSTAR velocidade
            motor_esq = vel_base - correcao
            motor_dir = vel_base + correcao
    ...
    if contours_green:
        for c in contours_green:
            ...
            if area > 1000: #AJUSTAR a área/tamanho min do verde
