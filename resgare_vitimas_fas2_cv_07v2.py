...
while True:
    ...
    for circles, color in detections:
        ...
        for c in circles[0, :]:
            all_circles.append((c, color))

    if len(all_circles) > 0:

        chosen, color = max(all_circles, key=lambda item: item[0][2])
        ...
        if stable_target is None:
            ...
            d = dist(current_center[0], current_center[1],
        ...
        if stable_count >= stable_frames:
            cv2.putText(frame,"ALVO CONFIRMADO",(50, 50),cv2.FONT_HERSHEY_SIMPLEX,1,(0, 255, 0),2)
