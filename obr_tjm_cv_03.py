...
while True:
    ...
    if dt <= 0:
        dt = 0.0001
    #line contours
    contours_black, _ = cv2.findContours(mask_black_line, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #funcao da biblioteca CV para contornos apos o filtro da linha

    #cofig do mapa visual e do filtro/mask preto/verde linha --
    hsv_line = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) #converte imagem para HSV (tonalidade, saturacao, valor)
    mask_black_line = cv2.inRange(hsv_line, black_low_line, black_high_line)
    mask_green = cv2.inRange(hsv_line, green_low, green_high)
    #kernel = np.ones((4,4), np.uint8)
    #mask_black = cv2.morphologyEx(mask_black, cv2.MORPH_CLOSE, kernel)
    #mask_black = cv2.morphologyEx(mask_black, cv2.MORPH_OPEN, kernel)    

    #rsg masks --
    hsv_rsg = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV) #filtro
    #config da mask/filtro prata --
    silver_mask = cv2.inRange(hsv_rsg, silver_low, silver_high) #cria mask/filtro prata
    ...
    black_mask = cv2.inRange(hsv_rsg, black_low_rsg, black_high_rsg)
    kernel = np.ones((3,3), np.uint8)
    ...
    if rg == 0: #se o robo ja chegou na area de resgate
        for circles, color in detections:
            ...
            if stable_count >= stable_frames:
                ...
                while chosen[2] <= 50: ############################################################
                    ...
                    motor_esq.on(SpeedDPS(vel_esq)) # type: ignore
                    motor_dir.on(SpeedDPS(vel_dir)) # type: ignore
                motor_fr_pg(motor_fr_br, motor_fr_gr) #pega vitima
                vitimas += 1 #adiciona ao contador
                stable_count = 0
                stable_target = None
                center_cal = None
                motor_esq.on_for_seconds(SpeedDPS(90), 1.5) # type: ignore
                motor_dir.on_for_seconds(SpeedDPS(-90), 1.5) # type: ignore
        
        if vitimas >= 5 or timer >= 120: #numero de vitimas antes de sair do resgate
            rg=1
            #sair da area de resgate
        
    elif rg == 1: #robo está fora do resgate
        #LINHA
